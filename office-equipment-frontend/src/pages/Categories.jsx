...
const Categories = () => {
  ...
  const { values, setValues, handleChange, handleBlur, resetForm } = useForm({ name: '', description: '', managingDepartment: '' });
  ...
  const handleEdit = (category) => {
    ...
    setValues({ name: category.name, description: category.description || '', managingDepartment: category.managingDepartment || '' });
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6">
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400">Kelola kategori peralatan</p>
        </div>
        {isAdmin && (
        ...
        {filteredCategories.map((category) => (
          <div key={category.id} className="card p-5 group hover:shadow-lg transition-all">
              ...
              <div className="flex-grow">
                ...
                <div className="flex flex-col">
                  {category.managingDepartment && (
                    <p className="text-[9px] font-medium text-primary-500 bg-primary-50 dark:bg-primary-900/20 px-1.5 py-0.5 rounded-md self-start mt-1">
                      Diterima/Dikelola: {category.managingDepartment}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="flex justify-end gap-2 pt-4 border-t border-gray-50 dark:border-slate-800">
              {isAdmin && (
                <>
                  <button onClick={() => handleEdit(category)} className="p-1.5 text-primary-600 hover:bg-primary-50 rounded-lg"><FiEdit2 size={16} /></button>
                  <button onClick={() => handleDeleteClick(category)} className="p-1.5 text-primary-600 hover:bg-primary-50 rounded-lg"><FiTrash2 size={16} /></button>
                </>
              )}
            </div>
          </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Nama Kategori</label>
                  <input name="name" value={values.name} onChange={handleChange} className="input-field" placeholder="misal: Elektronik" required />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Departemen Pengelola</label>
                  <input
                    name="managingDepartment"
                    value={values.managingDepartment}
                    onChange={handleChange}
                    className="input-field"
                    placeholder="misal: IT, GARAGE, LOGISTIK"
                  />
                  <p className="text-[10px] text-gray-400 mt-1 italic">* Petugas dari departemen ini yang akan mengurus peminjaman untuk kategori ini.</p>
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Deskripsi</label>
                  <textarea name="description" value={values.description} onChange={handleChange} className="input-field min-h-[100px]" placeholder="Penjelasan singkat mengenai kategori ini..." />
                </div>
